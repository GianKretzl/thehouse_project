  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div className="space-y-3">
        {/* Sidebar Variant */}
        <div>
          <Label className="text-sm font-medium">Variante da Barra Lateral</Label>
          {sidebarConfig.variant && (
            <p className="text-xs text-muted-foreground mt-1">
              {sidebarConfig.variant === "sidebar" && "Padrão: Layout de barra lateral padrão"}
              {sidebarConfig.variant === "floating" && "Flutuante: Barra lateral flutuante com borda"}
              {sidebarConfig.variant === "inset" && "Embutida: Barra lateral embutida com cantos arredondados"}
            </p>
          )}
        </div>
      ...
      <div className="space-y-3">
        <div>
          <Label className="text-sm font-medium">Modo de Recolhimento</Label>
          {sidebarConfig.collapsible && (
            <p className="text-xs text-muted-foreground mt-1">
              {sidebarConfig.collapsible === "offcanvas" && "Fora da Tela: Desliza para fora da vista"}
              {sidebarConfig.collapsible === "icon" && "Ícone: Recolhe apenas para ícones"}
              {sidebarConfig.collapsible === "none" && "Nenhum: Sempre visível"}
            </p>
          )}
        </div>
        <div className="grid grid-cols-3 gap-3">
      ...
      <div className="space-y-3">
        <div>
          ...
          {sidebarConfig.side && (
            <p className="text-xs text-muted-foreground mt-1">
              {sidebarConfig.side === "left" && "Esquerda: Barra lateral posicionada à esquerda"}
              {sidebarConfig.side === "right" && "Direita: Barra lateral posicionada à direita"}
            </p>
          )}
        </div>
